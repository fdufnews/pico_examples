# st7735b.py Driver for 0.96" 80*160 ST7735S LCD displays
# the blue tab display has a 24 lines offset
# the contrast is inverted
#based on st7735r.py Driver of Peter Hinch
...
class ST7735B(framebuf.FrameBuffer):
    ...
    def _init(self, usd):
        ...
        cmd(b'\x21') # INVON modif
        # d7..d5 of MADCTL determine rotation/orientation
        if self.height > self.width:
            wcd(b'\x36', b'\xc0' if usd else b'\x00')  # MADCTL: RGB portrait mode
            wcd(b'\x2a', int.to_bytes(self.width + 24 - 1 + (24 << 16), 4, 'big'))  # CASET column address 0 start, 160 end
            wcd(b'\x2b', int.to_bytes(self.height - 1, 4, 'big'))  # RASET
        else:
            wcd(b'\x36', b'\xa0' if usd else b'\x60')  # MADCTL: RGB landscape mode
            wcd(b'\x2a', int.to_bytes(self.width - 1, 4, 'big'))  # CASET column address 0 start, 160 end
            wcd(b'\x2b', int.to_bytes(self.height + 24 - 1 + (24 << 16), 4, 'big'))  # RASET
